...
class DialogueBoxPsych extends FlxSpriteGroup
{
	...
	public function new(dialogueList:DialogueFile, ?song:String = null, ?canSkip:Bool = false)
	{
		super();
		...
		skipText = new FlxText(0, FlxG.height - 30, FlxG.width, 'Press ${CoolUtil.getKey('accept')} to Continue', 16);
		if(canSkip)
			skipText.text += ' / ${Language.getPhrase('dialogue_skip', 'Press BACK to Skip').replace('BACK', CoolUtil.getKey('back'))}';
	...
	override function update(elapsed:Float)
	{
		...
		if(!dialogueEnded) {
			...
			if(daSound != null) daSound.destroy();
	...
	function startNextDialog():Void
	{
		...
		daText.volume = 0.2;
		if(curDialogue.audio != null && #if MODS_ALLOWED !FileSystem.exists('shared/sounds/dialogue/${PlayState.instance.songName}/${curDialogue.audio}') #else !OpenFlAssets.exists('shared/sounds/dialogue/${PlayState.instance.songName}/${curDialogue.audio}') #end) {
			daSound = new FlxSound().loadEmbedded(Paths.sound('dialogue/${PlayState.instance.songName}/${curDialogue.audio}'));
			daSound.autoDestroy = true;
			daSound.play();
		}
