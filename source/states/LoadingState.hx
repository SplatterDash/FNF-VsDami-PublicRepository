...
class LoadingState extends MusicBeatState
{
	...
	override function create()
	{
		...
		funkay = new FlxSprite(0, 0).loadGraphic(Paths.image('loading_screen/backgrounds/bg-1'));
	...
	public static function prepare(images:Array<String> = null, sounds:Array<String> = null, music:Array<String> = null, videos:Array<String> = null)
	{
	...
	static function _startPool()
	{
		...
		if(ClientPrefs.data.multiThread) threadCount = Std.int(Math.max(1, getCPUThreadsCount() - #if DISCORD_ALLOWED 2 #else 1 #end))
			else
		#end
		threadCount = 1;
					...
					for (asset in Reflect.fields(json))
					{
						...
						if(filters < 0 || StageData.validateVisibility(filters))
						{
							if(asset.startsWith('images/'))
								...
								preloadCharacter(asset.substr('characters/'.length));
						}
					}
	...
	static function clearInvalidFrom(arr:Array<String>, prefix:String, ext:String, type:AssetType, ?parentFolder:String = null)
	{
		...
		while(i < arr.length)
		{
			...
			if(member.endsWith('/') || (!Paths.fileExists(myKey, type, false, parentFolder) && (doTrace = true)))
			{
				arr.remove(member);
				if (doTrace)
					trace('Removed invalid $prefix: $member ; path: ${Paths.getPath(myKey, type)}');
			}
			else i++;
		}
	}
