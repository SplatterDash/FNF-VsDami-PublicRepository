		...
		for (i => option in options)
		{
			if(onKeyboardMode || option[0])
			{
				if(option.length > 1)
				{
					...
					text.targetY = myID;
					if(!text.bold) for(letter in text.letters) letter.setColorTransform(0, 0, 0, 1, 255, 255, 255);
					text.ID = myID;
					lastID = myID;
		...
		if(!binding)
		{
			...
			if(FlxG.keys.justPressed.ENTER || FlxG.gamepads.anyJustPressed(START) || FlxG.gamepads.anyJustPressed(A))
			{
				if(options[curOptions[curSelected]][1] != defaultKey)
				{
					...
					for(letter in bindingText.letters) letter.setColorTransform(0, 0, 0, 1, 255, 255, 255);
					add(bindingText);
