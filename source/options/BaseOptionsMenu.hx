...
class BaseOptionsMenu extends MusicBeatSubstate
{
	...
	private var grpBoxes:FlxTypedGroup<FlxSprite>;
	...
	public function new()
	{
		...
		add(new FlxSprite().loadGraphic(Paths.image('menus/extras/wedge')));

		// avoids lagspikes while scrolling through menus!
		grpBoxes = new FlxTypedGroup<FlxSprite>();
		add(grpBoxes);
		...
		for (i in 0...optionsArray.length)
		{
			...
			for (letter in optionText.letters) letter.setColorTransform(0, 0, 0, 1, 255, 255, 255);
			optionText.isMenuItem = true;
			optionText.targetY = i;
			optionText.distancePerItem.x = 110;
			grpOptions.add(optionText);

			var optionBox = new AttachedSprite('menus/extras/itemrectangle');
			optionBox.sprTracker = optionText;
			optionBox.xAdd = -525;
			optionBox.yAdd = 40;
			optionBox.copyAlpha = true;
			optionText.ID = i;
			grpBoxes.add(optionBox);
			...
			else
			{
				...
				valueText.ID = i;
				for(letter in valueText.letters) letter.setColorTransform(0, 0, 0, 1, 255, 255, 255);
	...
	override function update(elapsed:Float)
	{
		...
		if(FlxG.mouse.visible) for(item in grpBoxes) {
			if(FlxG.mouse.overlaps(item) && grpBoxes.members.indexOf(item) != curSelected && FlxG.mouse.justPressed) changeSelection(grpBoxes.members.indexOf(item), true);
		}
		...
		if(nextAccept <= 0)
		{
			...
			{
				case BOOL:
					if(controls.ACCEPT || (allowMouse && FlxG.mouse.overlaps(grpBoxes.members[curSelected]) && FlxG.mouse.justPressed))
					{
	...
	function updateBind(?text:String = null, ?option:Option = null)
	{
		...
		option.child = attach;
		for(letter in attach.letters) letter.setColorTransform(0, 0, 0, 1, 255, 255, 255);
	...
	function updateTextFrom(option:Option) {
		...
		if(option.child != null) for(letter in option.child.letters) letter.setColorTransform(0, 0, 0, 1, 255, 255, 255);
	}
	
	function changeSelection(change:Int = 0, ?force:Bool = false)
	{
		curSelected = FlxMath.wrap(force ? change : curSelected + change, 0, optionsArray.length - 1);
