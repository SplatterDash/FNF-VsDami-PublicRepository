...
class LanguageSubState extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		var bg = new FlxSprite().loadGraphic(Paths.image('menubackgrounds/backgroundDesat'));
		bg.color = 0xff7f71fd;
	...
	override function update(elapsed:Float)
	{
		...
		if(allowMouse) {
			...
			for(item in grpLanguages)
				if(FlxG.mouse.overlaps(item) && grpLanguages.members[curSelected] != item) changeSelected(grpLanguages.members.indexOf(item), true);
		...
		if(controls.ACCEPT || (FlxG.mouse.overlaps(grpLanguages.members[curSelected]) && FlxG.mouse.pressed))
		{
	...
	function changeSelected(change:Int = 0, force:Bool = false)
	{
		curSelected = FlxMath.wrap(force ? change : (curSelected + change), 0, languages.length-1);
