...
class VisualsSettingsSubState extends BaseOptionsMenu
{
	...
	public function new()
	{
		...
		var option:Option = new Option('Pause Music:',
			...
			'pauseMusic',
			STRING,
			['None', 'Tea Time', 'Breakfast', 'Breakfast (Pico)', 'Dami Themes']);
		addOption(option);
		...
		#if desktop
		var option:Option = new Option('Use Mouse for UI',
		...
		if (FlxG.save.data.completedDemo) {
			var option:Option = new Option('Mouse Texture',
				'Congratulations! What would you like your mouse to look like?',
				'mouseTex',
				STRING,
				['Default Mouse', 'Dami Mouse']);
			addOption(option);
			option.onChange = onChangeMouseTex;
		}
		#end
	...
	override function changeSelection(change:Int = 0, ?force:Bool = false)
	{
	...
	function onChangeMouse() {
		...
		ClientPrefs.data.mouseUI = allowMouse;
	}

	function onChangeMouseTex() {
		var newTex:String = 'defaultmouse';
		if(curSelected == 1) newTex = 'damimouse';
		FlxG.mouse.load('assets/shared/images/mice/$newTex.png', 1, 0, 0);
	}
	...
	{
		if(changedMusic && !OptionsState.onPlayState) FlxG.sound.playMusic(Paths.music('options'), 1, true);
