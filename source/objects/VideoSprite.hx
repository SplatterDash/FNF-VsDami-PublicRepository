...
class VideoSprite extends FlxSpriteGroup {
	...
	public function new(videoName:String, isWaiting:Bool, canSkip:Bool = false, shouldLoop:Dynamic = false, displayText:Bool = false) {
		super();
		...
		if(displayText) {
			skipText = new FlxText(0, 0, 0, 'Press and hold ${!Controls.instance.controllerMode ? InputFormatter.getKeyName(backend.ClientPrefs.keyBinds.get('accept')[0]).toUpperCase() : InputFormatter.getGamepadName(backend.ClientPrefs.gamepadBinds.get('accept')[0]).toUpperCase()} to skip');
			skipText.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, CENTER, OUTLINE, FlxColor.BLACK);
			skipText.screenCenter(X);
			skipText.y = camera.viewBottom - skipText.height - 10;
			skipText.alpha = 0;
			add(skipText);
		}
		...
		videoSprite.bitmap.onFormatSetup.add(function()
		{
			...
			#end
			*/
			videoSprite.setGraphicSize((videoSprite.width > videoSprite.height) ? FlxG.width : 0, (videoSprite.width < videoSprite.height) ? FlxG.height : 0);
		...
		if(skipText != null) {
			var daTween:FlxTween = FlxTween.tween(skipText, { alpha: 1 }, 1, { ease: FlxEase.quadInOut, type: PINGPONG, loopDelay: 5 });
