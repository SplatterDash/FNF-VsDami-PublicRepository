	...
	private function set_noteType(value:String):String {
		...
		if(noteData > -1 && noteType != value) {
			switch(value) {
				...
				case 'GF Sing':
					gfNote = true;
				case 'DOMINATION Hurt Note':
					reloadNote('HURTDOMINATION_assets');
					rgbShader.enabled = false;
					ignoreNote = mustPress;

					// gameplay data
					lowPriority = true;
					missHealth = FlxG.random.float(0.1, 1) / (isSustainNote ? 4 : 1);
					hitCausesMiss = true;
					hitsound = 'cancelMenu';
					hitsoundChartEditor = false;
				case 'Reawaken Hurt Note':
					reloadNote('HURTREAWAKEN_assets');
					rgbShader.enabled = false;
					ignoreNote = mustPress;
					...
					hitsoundChartEditor = false;
				case 'Health Note':
					reloadNote('HEALTHNOTES_assets');
					rgbShader.enabled = false;

					// gameplay data
					lowPriority = true;
					hitHealth = FlxG.random.float(0.25, 1);
					hitsound = 'confirmMenu';
					hitsoundChartEditor = false;
				case 'Beatbox':
					animSuffix = '-bb';
			}
