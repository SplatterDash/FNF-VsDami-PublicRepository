...
class Character extends FlxSprite
{
	...
	**/
	...
	//for Alissa sprite/Twinsomnia mechanic
	...
	override function update(elapsed:Float)
	{
		...
		switch(curCharacter)
		{
			...
			case 'erectpico':
				if(animationNotes.length > 0 && Conductor.songPosition > animationNotes[0][0])
				{
					if((animationNotes[0][0] + animationNotes[0][2]) - Conductor.songPosition >= -50) playAnim(singAnimations[animationNotes[0][1]] + idleSuffix + (animationNotes[0][3] != null && animationNotes[0][3] == "Alt Animation" ? '-alt' : ''), true)
					else animationNotes.shift();
				}
				//if(isAnimationFinished() && getAnimationName().startsWith('sing')) dance();
		}
	...
	public function recalculateDanceIdle() {
		...
		danceIdle = ((hasAnimation('danceLeft' + idleSuffix) && hasAnimation('danceRight' + idleSuffix)) || curCharacter == 'alissagf') && !forceIdle;

		if(settingCharacterUp)
		{
			danceEveryNumBeats = (danceIdle ? 1 : 2);
			if (curCharacter == 'alissagf')
				idleSuffix = '-left';
		}
