...
class PauseSubState extends MusicBeatSubstate
{
	...
	override function update(elapsed:Float)
	{
		...
		var accepted:Bool = controls.ACCEPT || (allowMouse && FlxG.mouse.overlaps(grpMenuShit.members[curSelected]) && FlxG.mouse.pressed);
		...
		if(allowMouse) {
			...
			for(item in grpMenuShit)
				if(FlxG.mouse.overlaps(item) && FlxG.mouse.pressed) changeSelection(grpMenuShit.members.indexOf(item), true);
	...
	{
		curSelected = FlxMath.wrap(force ? change : (curSelected + change), 0, menuItems.length - 1);
